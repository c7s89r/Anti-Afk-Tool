...
VK_MAP = {
    ...
    **{f"numpad {i}": 0x60 + i for i in range(10)},
    ...
    "home": 0x24, "end": 0x23, "page up": 0x21, "prior": 0x21,
    "page down": 0x22, "next": 0x22, "insert": 0x2D, "delete": 0x2E,
    "shift": 0x10, "ctrl": 0x11, "control": 0x11, "alt": 0x12,
    "left shift": 0xA0, "right shift": 0xA1,
    "left ctrl": 0xA2, "right ctrl": 0xA3,
    "left alt": 0xA4, "right alt": 0xA5,
    "left windows": 0x5B, "win": 0x5B, "right windows": 0x5C, "apps": 0x5D,
...
class MovementSpammer:
    ...
    def _build_ui(self):
        ...
        self.header_canvas = tk.Canvas(self.root, width=360, height=80, bg=BG2, highlightthickness=0)
        self.header_canvas.pack(fill="x")
        self._scan_line = self.header_canvas.create_rectangle(0, 0, 360, 2, fill="#2a1a4a", outline="")
        self._title_txt = self.header_canvas.create_text(180, 100, text="nzv's macro", font=tf, fill=ACCENT2, anchor="center")
        self.header_canvas.create_text(180, 65, text="S -> W -> A -> D", font=sf, fill=DIM, anchor="center")
        ...
        self.wasd_canvas = tk.Canvas(vis_frame, width=320, height=60, bg=BG, highlightthickness=0)
        ...
        self.key_lbl = tk.Label(hk_row, textvariable=self.key_var, font=kf, fg=WHITE, bg=BG3,
                                width=6, relief="flat", pady=10,
                                highlightbackground=DIM, highlightthickness=1)
        self.key_lbl.pack(side="left")
        self.set_btn = tk.Button(hk_row, text="SET KEY", font=bf, fg=BG, bg=ACCENT2,
        ...
        self.hint_lbl = tk.Label(hk_frame, textvariable=self.hint_var, font=sf, fg="#998800", bg=BG)
        ...
        self.orb_canvas = tk.Canvas(st_frame, width=18, height=18, bg=BG, highlightthickness=0)
        self.orb_canvas.pack(side="left")
        self._orb = self.orb_canvas.create_oval(2, 2, 16, 16, fill=DIM, outline="")
        self.status_var = tk.StringVar(value="IDLE")
        self.status_lbl = tk.Label(st_frame, textvariable=self.status_var, font=stf, fg=MUTED, bg=BG)
        self.status_lbl.pack(side="left", padx=(6, 0))

        if not LIBS_OK:
            tk.Label(self.root, text=" pip install pyautogui", font=sf, fg="#ff5555", bg=BG).pack(pady=4)
        ...
        self._credits_dot = tk.Label(credits_frame, text=". . .", font=cf, fg=BG, bg=BG)
    ...
    def _capture_key(self):
        ...
        if KEYBOARD_LIB:
            try:
                ...
                if sc:
                    raw_vk = user32.MapVirtualKeyW(sc, 1)
            except Exception:
                pass
        else:
